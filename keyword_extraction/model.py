...
class KELightningModel(pl.LightningModule):
    def __init__(self, cfg, from_scratch, inference=False):
        ...
        if from_scratch:
            ...
            if inference:
                model_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), cfg["models_folder"] + self.model_name )
            else:
                # this is to continue fine-tuning the model from the previous task
                model_path = "sentence_similarity/models/" + self.model_name 
    ...
    def configure_optimizers(self):
        return Adam(self.parameters(), lr=self.lr, betas=self.betas, eps=self.eps)
...
def run_trainer(dm, config, from_scratch, inference=False):
    train_dataloader = dm.train_dataloader()
    val_dataloader = dm.val_dataloader()
    test_dataloader = dm.test_dataloader()
    model = KELightningModel(config, from_scratch, inference)
    model_checkpoint = pl.callbacks.ModelCheckpoint(monitor="val_f1", mode="max")
    callbacks=[
        pl.callbacks.EarlyStopping(monitor="val_f1", patience=3, mode="max"),
        model_checkpoint,
    ]
